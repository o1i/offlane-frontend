...
const BlockRep = ({block, cl, txt}: {block: Block, cl: string, txt: string}) => {
    ...
    return(
        <>
            <Box style={{top: Math.round(start * 100) + "%", height:  Math.round((end - start) * 100) + "%"}} className={cl}>
                {txt}
            </Box>
        </>
    )
...
export const Stundenplan = ({blocks}: {blocks: Block[]}) => {
  const classes = useStyles();
  ...
  return (
    <Grid container direction="row" alignItems="stretch" spacing={0} wrap="nowrap" className={classes.root}>
        ...
        {timeBlocks.map((block, index)=>
                <BlockRep block={block} key={block.id} cl={classes.timeBox} txt={block.start}/>
            )}
        </Grid>
        <Grid item className={classes.day} >
            ...
            {blocks.filter(b => b.weekDay === 1).map((block, index)=>
                <BlockRep block={block} key={block.id} cl={classes.blockRep} txt=""/>
            )}
        </Grid>
        <Grid item className={classes.day} >
            <Box style={{top: Math.round((12-globalStart)/(globalEnd - globalStart) * 100) + "%", height: Math.round((1)/(globalEnd - globalStart) * 100) + "%"}} 
